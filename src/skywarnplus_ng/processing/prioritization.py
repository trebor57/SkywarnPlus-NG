...
class AlertPrioritizer:
    ...
    def assess_risk(self, alert: WeatherAlert) -> RiskAssessment:
        ...
        factors = self._identify_risk_factors(
            alert,
            {"impact": impact_score, "probability": probability_score, "urgency": urgency_score},
        )
    ...
    def _determine_risk_level(
        self, impact_score: float, probability_score: float, urgency_score: float
    ) -> str:
    ...
    def _generate_mitigation_actions(
        ...
        if risk_level in ["Very High", "High"]:
            ...
            actions.extend(
                ["Continue monitoring", "Prepare for potential escalation", "Document situation"]
            )
