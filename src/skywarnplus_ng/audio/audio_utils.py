"""
Modern audio processing utilities using soundfile + numpy.

Resampling uses linear interpolation (numpy) so installs avoid the large scipy wheel.
"""
...
def _resample_audio_array(data: np.ndarray, orig_sr: int, target_sr: int) -> np.ndarray:
    """
    Resample audio with linear interpolation (mono: 1D, stereo: shape (samples, 2)).
    """
    ...
    if num_samples < 1:
        raise ValueError(
            f"Cannot resample: resulting audio would be empty (target rate {target_sr} Hz)"
        )
    x_old = np.arange(len(data), dtype=np.float64)
    x_new = np.linspace(0, len(data) - 1, num=num_samples, endpoint=True, dtype=np.float64)
    if data.ndim == 1:
        out = np.interp(x_new, x_old, data.astype(np.float64))
        return out.astype(np.float32)
    if data.ndim == 2:
        channels = data.shape[1]
        out = np.empty((num_samples, channels), dtype=np.float32)
        for ch in range(channels):
            out[:, ch] = np.interp(x_new, x_old, data[:, ch].astype(np.float64)).astype(np.float32)
        return out
    raise ValueError(f"Unsupported audio shape for resampling: {data.shape}")
...
class AudioData:
    ...
    def set_frame_rate(self, target_rate: int) -> "AudioData":
        ...
        try:
            resampled = _resample_audio_array(self.data, self.sample_rate, target_rate)
            logger.debug(
                f"Resampled audio from {self.sample_rate}Hz to {target_rate}Hz "
                f"({len(self.data)} -> {len(resampled)} samples)"
            )
    ...
    def _export_to_ulaw(self, audio: "AudioData", output_path: Path) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Exported audio to ulaw: {output_path} ({output_path.stat().st_size} bytes)"
            )
    ...
    def from_wav(file_path: str) -> "AudioData":
        ...
        try:
            ...
            logger.debug(
                f"Loaded WAV file: {file_path} ({len(data)} samples, {sample_rate}Hz, {channels} channels)"
            )
            return AudioData(data, sample_rate, channels)
        except sf.LibsndfileError as e:
            raise RuntimeError(
                f"Failed to read WAV file {file_path}: {e}. The file may be corrupted or in an unsupported format."
            ) from e
        except Exception as e:
    ...
    def from_mp3(file_path: str) -> "AudioData":
        ...
        try:
            ...
            subprocess.run(
                ["ffmpeg", "-y", "-i", str(file_path), str(temp_wav_path)],
                check=True,
                capture_output=True,
                timeout=30,
                text=True,
            ...
            if not temp_wav_path.exists():
                raise RuntimeError(
                    f"FFmpeg did not create output file when converting MP3: {file_path}"
                )
    ...
    def _from_ulaw(file_path: str) -> "AudioData":
        ...
        try:
            ...
            if not temp_wav_path.exists():
                raise RuntimeError(
                    f"FFmpeg did not create output file when converting ulaw: {file_path}"
                )
