...
class SkywarnPlusApplication:
    ...
    async def initialize(self) -> None:
        ...
        if self.config.scripts.alertscript_enabled:
            try:
                ...
                mappings = [
                    mapping.model_dump() for mapping in self.config.scripts.alertscript_mappings
                ]
                ...
                if self.config.scripts.alertscript_active_commands:
                    active_commands = [
                        cmd.model_dump() for cmd in self.config.scripts.alertscript_active_commands
                    ]
                if self.config.scripts.alertscript_inactive_commands:
                    inactive_commands = [
                        cmd.model_dump()
                        for cmd in self.config.scripts.alertscript_inactive_commands
                    ]
    ...
    def _initialize_processing_pipeline(self) -> None:
        ...
        self.alert_pipeline.add_processor(
            FilterProcessor(self.filter_chain), ProcessingStage.FILTERED
        )
        self.alert_pipeline.add_processor(
            DeduplicationProcessor(self.deduplicator), ProcessingStage.DEDUPLICATED
        )
        self.alert_pipeline.add_processor(
            PrioritizationProcessor(self.prioritizer), ProcessingStage.PRIORITIZED
        )
        self.alert_pipeline.add_processor(
            ValidationProcessor(self.validator), ProcessingStage.VALIDATED
        )
        self.alert_pipeline.add_processor(
            WorkflowProcessor(self.workflow_engine), ProcessingStage.WORKFLOW_EXECUTED
        )
    ...
    def _create_severe_alert_workflow(self) -> AlertWorkflow:
        ...
        notification_step = WorkflowStep(
            ...
            conditions=[{"type": "severity_gte", "severity": "Severe"}],
        )
        ...
        escalation_step = WorkflowStep(
            ...
            conditions=[{"type": "severity_equals", "severity": "Extreme"}],
        )
    ...
    async def _poll_cycle(self) -> None:
        ...
        try:
            ...
            # Poll timestamp reflects an actual NWS fetch attempt (not idle/no-county skips)
            ...
            if current_alerts is None:
                ...
                self.state["nws_last_error_message"] = (
                    ...
                    "The alert list below may be stale until the next successful poll."
                )
                self.state_manager.save_state(self.state)
                if self.web_dashboard:
            ...
            if self.alertscript_manager:
                try:
                    processed_events = await self.alertscript_manager.process_alerts(
                        current_alerts, self._previous_alert_events
                    )
    ...
    async def _fetch_alerts(self, county_codes: List[str]) -> Optional[List[WeatherAlert]]:
        ...
        try:
            ...
            if self.config.dev.inject_enabled:
                logger.info(
                    "DEV: Test alert injection enabled, generating tornado warning test alert"
                )
                # Auto-generate a tornado warning for all configured counties
                if county_codes:
                    tornado_alert_config = [
                        {"Title": "Tornado Warning", "CountyCodes": county_codes}
                    ]
                    injected_alerts = self.nws_client.generate_inject_alerts(
                        tornado_alert_config, county_codes
                    )
                    logger.info(
                        f"Generated {len(injected_alerts)} test tornado warning alerts for {len(county_codes)} counties"
                    )
    ...
    async def _handle_new_alerts(
        ...
        for alert in new_alerts:
            ...
            timer_id = (
                self.performance_logger.start_timer(f"alert_processing_{alert.id}")
            ...
            try:
                ...
                ) if self.alert_logger else logger.info(
                    f"New alert detected: {alert.event} (ID: {alert.id})"
                )
                    ...
                    try:
                        ...
                        async with PushOverNotifier(pushover_config) as pushover:
                            ...
                            if result.get("success", False):
                                ...
                                logger.warning(
                                    f"PushOver notification failed: {result.get('error', 'Unknown error')}"
                                )
                    except Exception as e:
                ...
                self.alert_logger.log_alert_processed(
                    ...
                    f"Processed new alert: {alert.event} ({alert.severity.value})"
                )
                ...
                if self.web_dashboard:
                    ...
                    await self.web_dashboard.broadcast_update(
                        "alerts_update", [alert.model_dump(mode="json")]
                    )
        ...
        for alert_id in expired_alert_ids:
            ...
            if self.audio_manager:
                try:
                    ...
                    if audio_cleaned > 0:
                        logger.debug(
                            f"Cleaned up {audio_cleaned} audio file(s) for expired alert {alert_id}"
                        )
                except Exception as e:
                        ...
                        if file_path.is_file():
                            try:
                                ...
                                if fnmatch.fnmatch(filename, f"desc_{alert_id}_*"):
                                    ...
                                    logger.debug(
                                        f"Cleaned up description file for alert {alert_id}: {file_path}"
                                    )
                            except OSError as e:
                                logger.warning(
                                    f"Failed to clean up description file {file_path}: {e}"
                                )
                    if cleaned_desc_count > 0:
                        logger.debug(
                            f"Cleaned up {cleaned_desc_count} description file(s) for expired alert {alert_id}"
                        )
            except Exception as e:
                logger.debug(
                    f"Could not clean up description files (SkyDescribe may not be enabled): {e}"
                )
    ...
    async def _handle_all_clear(self) -> None:
        ...
        # Log all-clear event
        self.alert_logger.log_all_clear() if self.alert_logger else logger.info(
            ...
            try:
                ...
                async with PushOverNotifier(pushover_config) as pushover:
                    ...
                    if result.get("success", False):
                        ...
                        logger.warning(
                            f"PushOver all-clear notification failed: {result.get('error', 'Unknown error')}"
                        )
            except Exception as e:
    ...
    def _should_announce_alert(self, alert: WeatherAlert) -> bool:
        ...
        if alert.id in last_sayalert:
            logger.debug(
                f"Alert {alert.id} ({alert.event}) already announced, skipping duplicate announcement"
            )
            return False
    ...
    def _filter_alert_counties(
        ...
        filtered_county_codes = [
            code for code in alert.county_codes if code in monitored_county_codes
        ]

        # If no monitored counties match, return original alert (shouldn't happen if filter worked)
        if not filtered_county_codes:
            logger.warning(
                f"Alert {alert.id} has no monitored counties after filtering, keeping original"
            )
            return alert
            ...
            for part in area_parts:
                ...
                for county_code, county_name in county_code_to_name.items():
                    if county_name:
                        ...
                        if (
                            part_lower == county_name_lower
                            or part_lower == county_name_lower.replace(" county", "")
                            or part_lower
                            == county_name_lower.replace(" county", "").replace(" ", "")
                        ):
        ...
        return alert.model_copy(
            update={"county_codes": filtered_county_codes, "area_desc": filtered_area_desc}
        )
        ...
        for county_code in county_codes:
            if county_code in county_code_map:
                ...
                if county_config.name and county_config.name.lower() in added_counties:
                    logger.debug(
                        f"County {county_code} ({county_config.name}) already added, skipping duplicate"
                    )
                    continue
                ...
                if county_config.audio_file:
                    ...
                    logger.info(
                        f"Using explicit audio file for {county_code}: {county_config.audio_file}"
                    )
                    ...
                    if not resolved_file:
                        ...
                        if resolved_file:
                            logger.info(
                                f"Generated county audio on demand for {county_config.name}: {resolved_file}"
                            )
                else:
                ...
                if resolved_file:
                    ...
                    if county_config.name:
                        logger.debug(
                            f"County audio file not found for {county_code} in {self.config.audio.sounds_path} (from county name: {county_config.name})"
                        )
            else:
                logger.debug(
                    f"County code {county_code} not found in configuration, will try name-based matching from area_desc"
                )

        # If area_desc is provided and we didn't find all counties, try matching by name from area_desc
        should_use_area_desc = bool(
            area_desc and (not county_codes or len(county_audio_files) < len(county_codes))
        )
        if should_use_area_desc:
            ...
            for area_name in area_names:
                ...
                if area_name_lower in county_name_map:
                    ...
                    area_name_no_county = re.sub(
                        r"\s+County\s*$", "", area_name_lower, flags=re.IGNORECASE
                    )
                ...
                if matched_county and matched_county.enabled:
                    logger.info(
                        f"Matched area name '{area_name}' to configured county '{matched_county.name}' (code: {matched_county.code})"
                    )
                    ...
                    if matched_county.audio_file:
                        ...
                        logger.info(
                            f"Using explicit audio file for area '{area_name}': {matched_county.audio_file}"
                        )
                    else:
                        ...
                        if not resolved_file:
                            ...
                            if resolved_file:
                                logger.info(
                                    f"Generated county audio on demand for area '{area_name}': {resolved_file} (using name '{preferred_name}')"
                                )
                    ...
                    if resolved_file:
                        ...
                        logger.debug(
                            f"Could not resolve county audio for area '{area_name}' (matched county: {matched_county.name})"
                        )
                else:
    ...
    def _generate_county_audio_if_missing(self, county_name: Optional[str]) -> Optional[str]:
        ...
        if not self.audio_manager:
            logger.debug(
                f"Cannot generate audio for '{county_name}' because audio manager is unavailable"
            )
            return None
        ...
        try:
            ...
            logger.error(
                f"Failed to generate county audio for '{county_name}': {exc}", exc_info=True
            )
            return None
    ...
    def _find_county_audio_file(self, county_name: str) -> Optional[str]:
        ...
        logger.debug(
            f"County audio file not found: tried {possible_filenames} in {self.config.audio.sounds_path} (from county name: {county_name})"
        )
        return None
    ...
    async def _announce_alert(self, alert: WeatherAlert) -> List[int]:
        ...
        logger.debug(
            f"Alert description: {alert.description[:100]}..."
            if alert.description and len(alert.description) > 100
            else f"Alert description: {alert.description}"
        )
        ...
        if self.config.alerts.with_county_names:
            ...
            county_audio_files = self._get_county_audio_files(
                county_codes_list, area_desc=alert.area_desc
            )
            logger.info(
        ...
        try:
            ...
            logger.info(
                f"Alert {alert.event} will be sent to {len(target_nodes)} nodes: {target_nodes}"
            )

            # Play audio on target nodes
            successful_nodes = await self.asterisk_manager.play_audio_on_nodes(
                audio_path, target_nodes
            )

            if successful_nodes:
                logger.info(
                    f"Alert audio playing on {len(successful_nodes)} nodes: {successful_nodes}"
                )
    ...
    async def _announce_all_clear(self) -> None:
        ...
        try:
            ...
            if successful_nodes:
                logger.info(
                    f"All-clear audio playing on {len(successful_nodes)} nodes: {successful_nodes}"
                )
